...
- **Moved the default container registry from a Docker Hub namespace to the Nokia-owned GitHub Container Registry (`ghcr.io/nokia`)**. The Helm chart now defaults `image.repository` to `ghcr.io/nokia/ncm-issuer` (and the troubleshooting sidecar to `ghcr.io/nokia/ncm-issuer-utils`) and the release workflow publishes to `ghcr.io/nokia/ncm-issuer` using the built-in `GITHUB_TOKEN` instead of personal Docker Hub credentials. **Migration**: pull images from `ghcr.io/nokia/ncm-issuer`. Deployments that already override `image.repository` are unaffected
