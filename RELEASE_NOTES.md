...
- **Added a third-party notices document** (`THIRD_PARTY_NOTICES.md`) listing each direct dependency with its pinned version and licence, marking the two that are test-only rather than shipped in the binary, and naming the base images the published containers are built on
