...
- **Container base images are now pinned to a digest**, so a given ncm-issuer release always builds on the same base. The controller image previously used `alpine:latest`, which meant two builds of the same source could ship different Alpine packages. The runtime base is now Alpine 3.24.1, the build stage is Go 1.26.8 and the troubleshooting sidecar is Rocky Linux 8, each pinned by digest and tracked by Dependabot
